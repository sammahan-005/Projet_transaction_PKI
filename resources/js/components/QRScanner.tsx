...
/**
 * Props du composant QRScanner
 */
interface QRScannerProps {
  /** Callback appelé lorsqu'un code QR est scanné avec succès */
  onScan: (data: string) => void;
  /** Callback appelé pour fermer le scanner */
  onClose: () => void;
}
 ...
 * Composant QRScanner
 * 
 * Composant modal pour scanner des codes QR avec la caméra de l'appareil.
 * Utilise la bibliothèque @yudiel/react-qr-scanner pour accéder à la caméra
 * et décoder les codes QR.
 * 
 * Fonctionnalités :
 * - Accès à la caméra arrière de l'appareil
 * - Décodage automatique des codes QR
 * - Gestion des erreurs d'accès à la caméra
 * - Interface utilisateur avec overlay de cadrage
 * 
...
const QRScanner = memo(({ onScan, onClose }: QRScannerProps) => {
  // État pour gérer les erreurs d'accès à la caméra
  const [error, setError] = useState<string | null>(null);

  /**
   * Gère le résultat du scan d'un code QR
   * 
   * Cette fonction est appelée automatiquement par le scanner lorsqu'un
   * code QR est détecté et décodé avec succès.
   * 
   * @param result Le résultat du scan (tableau d'objets avec rawValue)
   */
  const handleScan = (result: any) => {
    if (result && result[0]?.rawValue) {
      // Appeler le callback avec les données du code QR scanné
   ...
   * 
   * Cette fonction est appelée lorsqu'une erreur survient lors de l'accès
   * à la caméra ou du décodage d'un code QR.
   * 
